...
contract RelayManagerNotETH is Operatorable, ReentrancyGuard {
  ...
  constructor(
    ...
    require(_adminFee != 0, "RelayManagerNotETH: ADMIN_FEE_INVALID");
    require(_threshold >= 1, "RelayManagerNotETH: THRESHOLD_INVALID");
    require(_bridgeWallet != address(0), "RelayManagerNotETH: BRIDGE_WALLET_ADDRESS_INVALID");
    ...
    require(signerLength >= _threshold, "RelayManagerNotETH: SIGNERS_NOT_ENOUGH");
  ...
  function setAdminFee(
    ...
    require(newAdminFee != 0, "RelayManagerNotETH: ADMIN_FEE_INVALID");
    require(
      _verify(keccak256(abi.encodePacked(newAdminFee)), signatures),
      "RelayManagerNotETH: INVALID_SIGNATURE"
    );
  ...
  function setThreshold(
    ...
    require(newThreshold >= 1, "RelayManagerNotETH: THRESHOLD_INVALID");
    require(
  ...
  function setBridgeWallet(
    ...
    require(newBridgeWallet != address(0), "RelayManagerNotETH: BRIDGE_WALLET_ADDRESS_INVALID");
    require(
  ...
  function deposit(
    ...
    require(amount >= adminFee, "RelayManagerNotETH: DEPOSIT_AMOUNT_INVALID");
    require(receiver != address(0), "RelayManagerNotETH: RECEIVER_ZERO_ADDRESS");
    // Burn tokens
  ...
  function _send(
    ...
    require(receiver != address(0), "RelayManagerNotETH: RECEIVER_ZERO_ADDRESS");
    require(amount > adminFee, "RelayManagerNotETH: SEND_AMOUNT_INVALID");
    require(
      _verify(keccak256(abi.encodePacked(from, receiver, amount, nonce)), _signatures),
      "RelayManagerNotETH: INVALID_SIGNATURE"
    );
    require(!processedNonces[from][nonce], 'RelayManagerNotETH: TRANSFER_NONCE_ALREADY_PROCESSED');
