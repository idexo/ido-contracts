...
contract SingleUsePayments is IPayments, ReceiptToken, ReentrancyGuard {
    ...
    function payProduct(string memory productId) external override {
        ...
        require(product.openForSale, "Payments#payProduct: PRODUCT_UNAVAILABLE");
        _payProduct(msg.sender, productId, product.price, product.paymentToken);
    }
    ...
    function refund(uint256 receiptId_) external onlyOperator {
        ...
        require(!receipts[receiptId_].hasUsed, "ALREADY_USED");
    ...
    function _payProduct(
        ...
        require(pToken.allowance(msg.sender, address(this)) >= price_, "INSUFFICIENT_ALLOWANCE");
