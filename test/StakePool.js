    ...
    contract("::" + contractName, async (accounts) => {
        ...
        describe("#Role", async () => {
            ...
            describe("reverts if", async () => {
                it("add operator by non-admin", async () => {
                    await expectRevert(stakePool.addOperator(bob, { from: bob }), errorHead + "#onlyAdmin: CALLER_NO_ADMIN_ROLE")
                })
                it("remove operator by non-admin", async () => {
                    await stakePool.addOperator(bob, { from: alice })
                    await expectRevert(stakePool.removeOperator(bob, { from: bob }), errorHead + "#onlyAdmin: CALLER_NO_ADMIN_ROLE")
                })
            })
        })
        ...
        describe("#Stake", async () => {
            ...
            describe("##withdraw", async () => {
                it("should withdraw", async () => {
                    ...
                    await stakePool.withdraw(1, web3.utils.toWei(new BN(2600)), { from: alice })
                    await expectRevert(stakePool.getStakeInfo(1), "StakeToken#getStakeInfo: STAKE_NOT_FOUND")
                })
                describe("reverts if", async () => {
        ...
        describe("#Reward", async () => {
            describe("##deposit", async () => {
                ...
                it("sweep", async () => {
                    expectEvent(await stakePool.sweep(ido.address, alice, 1, { from: alice }), "Swept")
                })
                describe("reverts if", async () => {
                    it("deposit amount is 0", async () => {
                        await expectRevert(stakePool.depositReward(new BN(0), { from: alice }), errorHead + "#depositReward: ZERO_AMOUNT")
                    })
        ...
        describe("##distribute", async () => {
            ...
            it("getEligibleStakeAmount", async () => {
                // After 5 days
                ...
                await stakePool.getEligibleStakeAmount(latestBlock.timestamp).then((res) => {
                    expect(res.toString()).to.eq("35760000000000000000000")
                })
            })

            after(async () => {
                timeTraveler.advanceTime(time.duration.days(-32))
            })
        })
    })
}
