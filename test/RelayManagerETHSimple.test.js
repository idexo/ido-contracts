...
const RelayManagerETH = artifacts.require("RelayManagerETHSimple")
...
contract("RelayManagerETH", async (accounts) => {
    ...
    const ownerPrivateKey = Buffer.from("7c852118294e51e653712a81e05800f419141751be58f605c371e15141b007a6", "hex")
    ...
    describe("cross-chain transfer", async () => {
        ...
        it("permitAndDeposit", async () => {
            ...
            // Approve it
            expectEvent(await relayManager.permitAndDeposit(bob, 100, polygonChainId, permitOptions, { from: carol }), "Deposited")
        })
        it("send", async () => {
        ...
        it("withdraw admin fee", async () => {
            expectEvent(await relayManager.withdrawAdminFee(bob, adminFee), "AdminFeeWithdraw")
        })
        it("withdraw gas fee", async () => {
            expectEvent(await relayManager.withdrawGasFee(bob, gasFee), "GasFeeWithdraw")
        })
    ...
    describe("#Ownership", async () => {
        ...
        describe("reverts if", async () => {
            ...
            it("non-owner call setAdminFee", async () => {
                await expectRevert(relayManager.setAdminFee(1, { from: carol }), "RelayManagerETH: CALLER_NO_OWNER")
            })
